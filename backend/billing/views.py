...
def recent_purchases(request):
    """GET /api/billing/recent/ — Returns the last 10 successful premium subscription purchases."""
    ...
    data = []
    for sub in recent:
        plan_name = sub.plan.name.split(' (')[0] if sub.plan else "Premium"
        user_name = sub.user.full_name or sub.user.username
        
        is_org = sub.plan and sub.plan.plan_type == 'organization'
        center_name = None
        if is_org:
            from centers.models import EducationCenter
            center = EducationCenter.objects.filter(owner=sub.user).first()
            if center:
